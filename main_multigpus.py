#torchrun --nproc_per_node=2 --nnodes=1 main_multigpus.py
...
def main(test_flag=False, model_dir="models/"):
    ...
    for params in param_list:
        ...
        best_model_name = model_dir + "input_size_{}_prediction_size_{}_window_{}_hidden_dim_{}_num_layers_{}_best.pth".format(input_size , prediction_size , window , hidden_dim , num_layers)
        ...
        if val_min_loss < best_val_loss:
            ...
            state = {'model': model.module.state_dict() if isinstance(model, (torch.nn.DataParallel, torch.nn.parallel.DistributedDataParallel)) else model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch, 'scheduler': scheduler.state_dict()}
